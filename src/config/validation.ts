...
export const SubtaskDefinitionSchema = z.object({
  ...
  coin: z
    .number()
    .int()
    .min(0, 'Coin must be non-negative')
    .max(999999, 'Coin too large')
    .optional(),
  ...
  exp: z
    ...
    .max(99999, 'Experience too large')
    .optional(),
...
export const CreateTaskSchema = z
  .object({
    ...
    task_type: z
      .number()
      .int()
      .min(0, 'Task type must be 0-3')
      .max(3, 'Task type must be 0-3')
      .optional(),
  ...
  .refine(
    ...
    {
      message:
        'When task_type is 1 (count task), target_times must be provided and must be positive',
      path: ['target_times'],
    }
  );
...
export const SearchTasksSchema = z.object({
  ...
  searchQuery: z.string().max(200, 'Search query cannot exceed 200 characters').optional(),
...
export const CreateAchievementSchema = z
  .object({
    ...
    color: z
      .string()
      .regex(/^#[0-9A-Fa-f]{6}$/, 'Color must be hex format (e.g., #66CCFF)')
      .optional(),
  ...
  .refine(
    ...
    {
      message:
        'When exp is specified, skills must be provided as a non-empty array. Without skills, XP cannot be applied to any attributes.',
      path: ['skills'],
    }
  );
...
export const EditShopItemSchema = z
  .object({
...
// Edit Skill Schema
export const EditSkillSchema = z
  .object({
...
export const CreateSubtaskSchema = z
  .object({
    ...
    todo: z
      .string()
      .min(1, 'Subtask content is required')
      .max(200, 'Subtask content cannot exceed 200 characters'),
  ...
  .refine(
    (data) =>
      data.main_id !== undefined || data.main_gid !== undefined || data.main_name !== undefined,
    {
      message:
        'At least one of main_id, main_gid, or main_name must be provided to identify the parent task',
      path: ['main_id'],
    }
  );

// Edit Subtask Schema
export const EditSubtaskSchema = z
  .object({
    // Parent task identification (at least one required)
    main_id: z.number().int().positive('Main task ID must be positive').optional(),
    main_gid: z.number().int().positive('Main task group ID must be positive').optional(),
  ...
  .refine(
    (data) =>
      data.edit_id !== undefined || data.edit_gid !== undefined || data.edit_name !== undefined,
    {
      message:
        'At least one of edit_id, edit_gid, or edit_name must be provided to identify the subtask to edit',
      path: ['edit_id'],
    }
  );
