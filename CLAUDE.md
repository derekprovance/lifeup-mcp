...
#### Achievement Condition Updates May Fail Silently

**Issue**: When updating `conditions_json` on existing achievements via `update_achievement`, the LifeUp API returns success but may not apply the changes.

**Symptoms**:
- API responds with `{"code": 200, "data": "success"}`
- Achievement conditions remain unchanged in LifeUp app
- No error message or indication of failure

**Root Cause**: The LifeUp API appears to accept but ignore `conditions_json` during edit operations, despite documentation listing it as a valid parameter.

**Workarounds**:
1. **Delete and recreate**: Remove the achievement with `delete_achievement` and create a new one with conditions using `create_achievement`
2. **Set during creation**: Always include `conditions_json` when creating achievements with `create_achievement`, not after
3. **Verify manually**: Check in LifeUp app after any condition update to confirm changes were applied

**User Feedback**: The server now warns users when attempting condition updates and suggests these workarounds.

**Related Endpoints**:
- ✅ `create_achievement` with `conditions_json` - Works correctly
- ⚠️  `update_achievement` with `conditions_json` - May fail silently (API limitation)
- ℹ️  `unlock_condition` - Unrelated endpoint (only for triggering type 20 External API conditions that have already been created)
